...
public class ClientApp {
	...
	public void execute() {
		...
		commandMap.put("관리자로그인", new MemberAdminLoginHandler(stmt));
		loop:
			while(true) {
				if(location == -1) /* 메인 메뉴 */ {
					...
					commandMap.put("1", new MemberAddHandler(stmt, memberValidator));
					commandMap.put("2", new MemberLoginHandler(stmt));
					commandMap.put("3", new BuyerBoardServiceHandler(stmt));
					commandMap.put("4", new SellerBoardServiceHandler(stmt));
					commandMap.put("5", new IntegratedBoardServiceHandler(stmt));
